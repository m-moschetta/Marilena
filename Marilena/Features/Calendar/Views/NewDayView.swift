...
/// Vista giornaliera dettagliata con supporto corretto per eventi all-day
public struct NewDayView: View {
    ...
    public var body: some View {
        ScrollViewReader { proxy in
            VStack(spacing: 0) {
                ...
                // Sezione eventi all-day (se presenti)
                allDayEventsSection
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    ZStack(alignment: .topLeading) {
                        ...
                        // Eventi con orario (esclusi all-day)
                        timedEventsLayer
            ...
            // Gesture principale: pinch per zoom
            .simultaneousGesture(
            ...
            // Gesture secondaria: swipe orizzontale con alta priorità
            .simultaneousGesture(
    ...
    private var allDayEventsSection: some View {
        let allDayEvents = calendarService.dayData(for: calendarService.selectedDate)
            .filter { $0.isAllDay }
        ...
        return Group {
            if !allDayEvents.isEmpty {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    // Lista eventi all-day
                    VStack(spacing: 4) {
                        ForEach(allDayEvents) { event in
                            AllDayEventRow(event: event, calendarService: calendarService)
                        }
                    }
                    .padding(.leading, 41) // Allineato con la timeline
                }
                .padding(.horizontal, 20)
                .padding(.vertical, 8)
    ...
    // MARK: - Timed Events Layer (esclude all-day)
    private var timedEventsLayer: some View {
        let events = calendarService.dayData(for: calendarService.selectedDate)
            .filter { !$0.isAllDay } // Esclude eventi all-day
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 6)
                .fill(event.uiColor.opacity(0.15))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 6)
                .stroke(event.uiColor.opacity(0.3), lineWidth: 1)
        )
        .onTapGesture {
